...
class SyntheticDataset:
    ...
    def simulate_linear_sem(B, n, noise_type, rs=np.random.RandomState(1)):
        ...
        for i in ordered_vertices:
            ...
            X[:, i], N_i = _simulate_single_equation(X[:, parents], B[parents, i])
            N.append(N_i)
        return X, N


def dataset_based_on_B(B, n, B_scale, noise_type, seed=1):
    ...
    def simulate_weight(B_bin, B_ranges, rs=np.random.RandomState(1)):
        ...
        Args:
            B_bin (numpy.ndarray): [d, d] binary adjacency matrix of DAG.
            B_scale (tuple): Disjoint weight ranges.
            rs (numpy.random.RandomState): Random number generator.
                Default: np.random.RandomState(1).
    ...
    def simulate_linear_sem(W, n, noise_type, rs=np.random.RandomState(1)):
        """Simulate samples from linear SEM with specified type of noise.
        ...
        Returns:
            numpy.ndarray: [n, d] data matrix.
        """
        def _simulate_single_equation(X, W_i):
            """Simulate samples from linear SEM for the i-th node.
        ...
        for i in ordered_vertices:
            parents = list(G.predecessors(i))
            X[:, i] = _simulate_single_equation(X[:, parents], W[parents, i])

        return X

    W = simulate_weight(B, B_ranges)
    X = simulate_linear_sem(W, n, noise_type)
    return X
...
if __name__ == '__main__':
    ...
    X = dataset_based_on_B(B, n, B_scale, noise_type)
    print(X)